...
import {
  ...
  Edit3, ExternalLink, MessageCircle, Bell, IndianRupee, Star,
} from 'lucide-react';
...
const TIER: Record<string, { label: string; color: string }> = {
  high:   { label: 'Tier 1', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
  medium: { label: 'Tier 2', color: 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20' },
  low:    { label: 'Tier 3', color: 'text-slate-500 bg-slate-500/10 border-slate-500/20' },
};
...
export function LeadCard({ lead: initialLead, onUpdate, compact = false }: Props) {
  ...
  return (
    <div className={`bg-card border border-border rounded-xl overflow-hidden transition-all hover:shadow-md hover:shadow-black/20 ${overdue ? 'border-l-2 border-l-orange-500/70' : ''}`}>
      {/* Priority bar */}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                {/* Name + Status + Tier + Overdue */}
                <div className="flex items-center gap-1.5 flex-wrap">
                  <h3 className="font-semibold text-foreground text-sm">{lead.contact_name}</h3>
                  <StatusBadge status={lead.status} />
                  <span className={`inline-flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full border ${tier.color}`}>
                ...
                <p className="text-xs text-muted-foreground mt-0.5 truncate">
                  ...
                  {lead.contact_title && <><span className="text-border mx-1">·</span>{lead.contact_title}</>}
                </p>

                {/* Location + project + deal */}
                <div className="flex items-center gap-1 text-xs text-muted-foreground mt-0.5 flex-wrap">
                  ...
                  {lead.project_type && (
                    <>
                      ...
                      <span className="text-primary/70 text-[11px]">{lead.project_type}</span>
                    </>
                  )}
                  {lead.deal_value && (
                    <>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="h-8 px-2.5 text-xs font-medium rounded-lg bg-primary/15 text-primary hover:bg-primary/25 transition-colors flex items-center gap-1 border border-primary/20"
                >
            ...
            <div className="mt-3 space-y-2">
              ...
              {/* Follow-up + Remind row */}
              {lead.next_action_date && (
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`flex items-center gap-1.5 text-xs font-medium ${overdue ? 'text-orange-400' : 'text-muted-foreground'}`}>
                    <Calendar className="w-3 h-3" />
                    {overdue ? 'Overdue: ' : 'Follow up: '}{formatDate(lead.next_action_date)}
                  </span>
                  <a
                    href={`/api/calendar/${lead.id}`}
                    title="Add to iPhone Calendar / Reminders"
                  ...
                  {!waUrl && lead.email && (
                    <a href={`mailto:${lead.email}?subject=Follow up - Pongs Stretch Ceiling&body=Hi ${lead.contact_name.split(' ')[0]},`}
                      className="flex items-center gap-1.5 text-xs font-medium text-blue-400/80 hover:text-blue-400 bg-blue-500/10 border border-blue-500/20 rounded-full px-2.5 py-1 transition-colors">
