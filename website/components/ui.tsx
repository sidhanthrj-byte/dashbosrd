...
export function Section({
  ...
  return (
    <section className={`${dark ? "bg-ink text-paper" : ""} ${tight ? "py-14" : "py-20 md:py-28"}`}>
...
export function SectionHead({
  ...
  return (
    <div className="mb-12 max-w-3xl">
      ...
      <h2 className="display mt-3 text-3xl md:text-5xl">{title}</h2>
  ...
  return (
    <Link
      ...
      className={`inline-flex items-center gap-2 rounded-full px-7 py-3.5 text-sm font-semibold transition-colors ${
        ghost
          ? "border border-current hover:border-brass hover:text-brass"
      ...
      {items.map((f) => (
        <details key={f.q} className="faq group py-5">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-left text-base font-semibold md:text-lg">
            {f.q}
            <span className="text-2xl font-light text-brass transition-transform group-open:rotate-45">+</span>
          </summary>
